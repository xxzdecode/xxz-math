...
begin
  ...
  if row_value.client_hash is not null and row_value.blocked_until > current_time then
    return jsonb_build_object(
      'allowed', false,
      'retry_after_seconds', greatest(1, ceil(extract(epoch from row_value.blocked_until - current_time))::integer)
    );
  end if;
  ...
  if p_event = 'failure' then
    ...
    values (p_client_hash, current_time, 1, null)
    ...
    if row_value.blocked_until > current_time then
      return jsonb_build_object('allowed', false, 'retry_after_seconds', 900);
    end if;
  elsif row_value.client_hash is not null and current_time - row_value.window_started_at >= interval '15 minutes' then
    delete from public.math_teacher_rate_limit_v1 where client_hash = p_client_hash;
  end if;
  return jsonb_build_object('allowed', true, 'retry_after_seconds', 0);
end;
